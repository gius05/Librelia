...
WORKDIR /src
COPY ["Librelia.csproj", "."]
RUN dotnet restore "./Librelia.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./Librelia.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Librelia.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Librelia.dll"]